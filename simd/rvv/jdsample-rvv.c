                                   ...
                                   JSAMPARRAY *output_data_ptr)
{
    ...
    vuint16m4_t this_u16, shift_u16, p_odd_u16, p_even_u16;
    ...
    for (inrow = 0; inrow < max_v_samp_factor; inrow++)
    {
        ...
        *outptr++ = (JSAMPLE)GETJSAMPLE(*inptr);
        ...
        {
            vl = vsetvl_e16m4(incol - 1);

            /* Load smaples and samples with offset 1. */
            this = vle8_v_u8m2(inptr, vl);
            shift = vle8_v_u8m2(inptr + 1, vl);
            /* Widen to vuint16m4_t type. */
            this_u16 = vwaddu_vx_u16m4(this, 0, vl);
            shift_u16 = vwaddu_vx_u16m4(shift, 0, vl);

            /* p1(upsampled) = (3 * s0 + s1 + 1) / 4 */
            p_odd_u16 = vmul_vx_u16m4(this_u16, 3, vl);
            p_odd_u16 = vadd_vv_u16m4(p_odd_u16, shift_u16, vl);
            p_odd_u16 = vadd_vx_u16m4(p_odd_u16, 1, vl);    /* Add bias */
            /* p2(upsampled) = (3 * s1 + s0) / 4 */
            p_even_u16 = vmul_vx_u16m4(shift_u16, 3, vl);
            p_even_u16 = vadd_vv_u16m4(p_odd_u16, this_u16, vl);
...
/* FIXME: No support for "mf" suffix. */
    ...
    for (inrow = 0, outrow = 0; outrow < max_v_samp_factor; inrow++)
    {
        ...
        *outptr1++ = (JSAMPLE)((s0colsum1 * 4 + 8) >> 4);
             ...
             outptr0 += 2 * vl, outptr1 += 2 * vl)
        {
            ...
            /* 3 * s0A + s1A (row -1) */
            s0_add_3s1_sd = vmul_vx_u16m2(row_1_u16, 3, vl);
            s0_add_3s1_sd = vadd_vv_u16m2(s0_add_3s1_sd, row_1rs1_u16, vl);
            /* 3 * s1A + s0A (row -1) */
            s1_add_3s0_sd = vmul_vx_u16m2(row_1rs1_u16, 3, vl);
            s1_add_3s0_sd = vadd_vv_u16m2(s1_add_3s0_sd, row_1_u16, vl);
            /* 3 * (3 * s0B + s1B) (row 0) */
            s0_add_3s1_ctr = vmul_vx_u16m2(row0_u16, 3, vl);
            s0_add_3s1_ctr = vadd_vv_u16m2(s0_add_3s1_ctr, row0rs1_u16, vl);
            s0_add_3s1_ctr = vmul_vx_u16m2(s0_add_3s1_ctr, 3, vl);
            /* 3 * (3 * s1B + s0B) (row 0) */
            s1_add_3s0_ctr = vmul_vx_u16m2(row0rs1_u16, 3, vl);
            s1_add_3s0_ctr = vadd_vv_u16m2(s1_add_3s0_ctr, row0_u16, vl);
            s1_add_3s0_ctr = vmul_vx_u16m2(s1_add_3s0_ctr, 3, vl);

            /* Step 2: Blend the already-blended columns. */
            /* p13: (3 * (3 * s0B + s1B) + (3 * s0A + s1A) + 7) / 16 */
            p_odd0_u16 = vadd_vv_u16m2(s0_add_3s1_ctr, s0_add_3s1_sd, vl);
            p_odd0_u16 = vadd_vx_u16m2(p_odd0_u16, 7, vl);      /* Add bias */
            /* p14: (3 * (3 * s1B + s0B) + (3 * s1A + s0A) + 7) / 16 */
            p_even0_u16 = vadd_vv_u16m2(s1_add_3s0_ctr, s1_add_3s0_sd, vl);
            ...
            /* Step 1: Blend samples vertically in columns s0 and s1. */
            /* 3 * s0C + s1C (row 1) */
            s0_add_3s1_sd = vmul_vx_u16m2(row1_u16, 3, vl);
            s0_add_3s1_sd = vadd_vv_u16m2(s0_add_3s1_sd, row1rs1_u16, vl);
            /* 3 * s1C + s0C (row 1) */
            s1_add_3s0_sd = vmul_vx_u16m2(row1rs1_u16, 3, vl);
            s1_add_3s0_sd = vadd_vv_u16m2(s1_add_3s0_sd, row1_u16, vl);

            /* Step 2: Blend the already-blended columns. */
            /* p19: (3 * (3 * s0B + s1B) + (3 * s0C + s1C) + 7) / 16 */
            p_odd1_u16 = vadd_vv_u16m2(s0_add_3s1_ctr, s0_add_3s1_sd, vl);
            p_odd1_u16 = vadd_vx_u16m2(p_odd1_u16, 7, vl);      /* Add bias */
            /* p20: (3 * (3 * s1B + s0B) + (3 * s1C + s0C) + 7) / 16 */
            p_even1_u16 = vadd_vv_u16m2(s1_add_3s0_ctr, s1_add_3s0_sd, vl);
